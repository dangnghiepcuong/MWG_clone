<template>
    <div class="nav-slider">
        <div class="prev" @click="navigateSlider('#frame-slider-grid-product-sale', 'left', 5, 12, $event)">
            <div class="arrow arrow-left"></div>
        </div>
        <div class="next" @click="navigateSlider('#frame-slider-grid-product-sale', 'right', 5, 12, $event)">
            <div class="arrow arrow-right"></div>
        </div>
    </div>
    <div class="layout-product-sale">
        <div class="box-slider-relative">
            <div class="frame-slider" id="frame-slider-grid-product-sale">
                <ul class="slider">
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg"
                                    class=" lazyloaded" alt="HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)"
                                    src="https://cdn.tgdd.vn/Products/Images/44/302207/hp-pavilion-15-eg2086tu-i3-7c0q8pa-050223-080308-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    HP Pavilion 15 eg2086TU i3 1215U (7C0Q8PA)
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    11.490.000đ
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-24%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 10/10 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                    <li class="slide">
                        <a href="">
                            <div class="holder-label">
                                <span class="top-label">Trả góp 0%</span>
                            </div>
                            <div class="holder-img">
                                <img data-src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg"
                                    class=" lazyloaded" alt="Tai nghe Có dây EP AVA+ PC-25" height="207"
                                    src="https://cdn.tgdd.vn/Products/Images/54/325878/tai-nghe-co-day-ava-pc-25-1-600x600.jpg">
                            </div>
                            <div class="product-name">
                                <span>
                                    Tai nghe Có dây EP AVA+ PC-25
                                </span>
                            </div>
                            <div class="price">
                                <span>
                                    50.000₫
                                </span>
                            </div>
                            <div class="layout-original-price">
                                <span class="original-price">15.200.000đ</span>
                                <span class="discount">-50%</span>
                            </div>
                            <div class="layout-quantity">
                                <span class="icon">
                                    <img src="//cdnv2.tgdd.vn/webmwg/2024/ContentMwg/images/fs-iconfire.png"
                                        alt="icon flashsale">
                                </span>
                                <span class="quantity">Còn 9/30 suất</span>
                            </div>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</template>
<style scoped>
@import './flash-sale.css';
</style>